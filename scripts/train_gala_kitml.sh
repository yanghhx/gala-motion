...
if [[ "$STAGE" == "flow" ]]; then
  exec "$MAMBA" run -n gala-motion python "$ROOT/trainers/train.py" \
    --config "$ROOT/configs/gala_kitml_flow.yaml" \
    --init-from "$ROOT/checkpoints/gala_kitml_vae/best.pt" \
    "$@"
fi
if [[ "$STAGE" == "eval" ]]; then
  exec "$MAMBA" run -n gala-motion python "$ROOT/scripts/evaluate_t2m.py" \
    --config "$ROOT/configs/gala_kitml_flow.yaml" \
    --checkpoint "$ROOT/checkpoints/gala_kitml_flow/best.pt" \
    --protocol official \
    --replication-times 20 \
    --batch-size 32 \
    --output "$ROOT/checkpoints/gala_kitml_flow/official_eval.json" \
    "$@"
fi
exec "$MAMBA" run -n gala-motion python "$ROOT/trainers/train.py" \
  --config "$ROOT/configs/gala_kitml.yaml" \
  "$@"
